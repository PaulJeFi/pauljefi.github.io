...
\begin{theorem}
    Une matrice $A \in \mathcal{M}_{n}(\mathbb{R})$ est de rang $1$ si et seulement s'il existe deux vecteurs colonne $X, Y \in \mathcal{M}_{n, 1}(\mathbb{R})$ non-nuls tels que $A = XY^\top$.
\end{theorem}

\begin{proof}
    $\boxed{\Rightarrow}$ Soit $A \in \mathcal{M}_{n}(\mathbb{R})$ une matrice de rang $1$. On note $c_1, c_2, \dots, c_n$ ses colonnes. $A$ n'est pas la matrice nulle puisqu'elle est de rang $1$. Il existe alors $i \in \llbracket 1, n \rrbracket$ telle que $c_i$ n'est pas la colonne nulle. Il existe donc $\left(\lambda_k\right)_{k \in \llbracket 1, n \rrbracket} \in \mathbb{R}^{\llbracket 1, n \rrbracket}$ tel que pour tout $k \in \llbracket 1, n \rrbracket$, $c_k = \lambda_k c_i$. On a alors :
    ...
    Comme $c_i$ n'est pas la colonne nulle, alors $X$ est un vecteur colonne non-nul et $c_i = \lambda_i c_i$ d'où $\lambda_i = 1$, donc $Y$ est aussi un vecteur colonne non-nul.\\
